...
def evaluate_action_clustering(X, Y, actions, max_samples=5000):
    ...
    try:
        if len(np.unique(act)) > 1:
            # Need min classes check for train test split
            from collections import Counter
            counts = Counter(act)
            valid_classes = {k for k, v in counts.items() if v >= 2}
            
            if len(valid_classes) > 1:
                f_idx = [i for i, a in enumerate(act) if a in valid_classes]
                X_f = X[f_idx]
                Y_f = Y[f_idx]
                act_f = act[f_idx]
                
                X_train, X_test, y_train, y_test, Y_train, Y_test = train_test_split(X_f, act_f, Y_f, test_size=0.2, stratify=act_f)
...
def parse_skipped_actions(raw_actions):
    """ Extract string labels for skipped latents actions """
    parsed = []
    
    for i in range(len(raw_actions)):
        act_dict = raw_actions[i]
...
def main():
    parser = argparse.ArgumentParser(description="Compute CKA between Adaworld/Olafworld from skipped latents")
    parser.add_argument("--dir1", type=str, default="adaworld")
    parser.add_argument("--dir2", type=str, default="olafworld")
    args = parser.parse_args()

    base_dir1 = os.path.join('latent_actions_skipped', args.dir1)
    base_dir2 = os.path.join('latent_actions_skipped', args.dir2)
    
    files1 = glob.glob(os.path.join(base_dir1, "**", "latent_actions.pt"), recursive=True)
    print(f"Found {len(files1)} directories in {args.dir1}", flush=True)
    ...
    for f1 in files1:
        ...
        try:
            data1 = torch.load(f1, map_location='cpu', weights_only=False)
            data2 = torch.load(f2, map_location='cpu', weights_only=False)
        except Exception as e:
            continue
        ...
        if len(a1) >= min_len:
            actions_raw = a1[:min_len] 
        elif len(a2) >= min_len:
             actions_raw = a2[:min_len]
        else:
             actions_raw = []
             
        actions = parse_skipped_actions(actions_raw) if actions_raw else []

        parts = rel_path.split(os.sep)
        game_name = parts[0]
        
        # Additional filter: keep only elements that map to one of our 5 target action classes
        valid_idx = [i for i, a in enumerate(actions) if a is not None]
        if len(valid_idx) == 0:
            continue
        ...
        z1 = z1[valid_idx]
        z2 = z2[valid_idx]
        actions = [actions[i] for i in valid_idx]
