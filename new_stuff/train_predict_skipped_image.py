...
def extract_target(t_img):
    return torch.from_numpy(downscale_to_blocks(t_img)).float()

def load_data(model_name, force_extract=False):
    cache_z = f"latent_actions_skipped/{model_name}_z_mu_img.pt"
    cache_t = f"latent_actions_skipped/{model_name}_image_targets.pt"
