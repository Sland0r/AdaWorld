...
Supports two data formats:
  - Sharded .pt files (recommended): /scratch-shared/FoMo-Atomic-Actions/sharded_targets/difference/dump_dir_*/
  - Legacy per-sample .npy files: difference_dump/*/difference_step1/*.npy
"""
...
def get_file_list(base_dir, extension, cache_dir="/scratch-shared/FoMo-Atomic-Actions/_cache"):
    cache_path = os.path.join(cache_dir, "difference_files.txt")
...
def get_shard_dir(shard_root: str, dump_dir: int) -> str:
    return os.path.join(shard_root, "difference", f"dump_dir_{dump_dir}")


def get_shard_files(shard_dir: str) -> List[str]:
    return sorted(glob.glob(os.path.join(shard_dir, "difference_shard_*.pt")))
...
def train_legacy_npy(args, device, criterion):
    ...
    print("Loading difference file list...")
    diff_files = get_file_list(diff_base, "npy", cache_dir=args.cache_dir)
    print(f"Found {len(diff_files)} difference files")

    num_samples = min(z.shape[0], len(diff_files))
    z = z[:num_samples]
    diff_files = diff_files[:num_samples]
    print(f"Aligned to {num_samples} samples")

    sample = np.load(diff_files[0])
    ...
    train_files = [diff_files[i] for i in train_idx.tolist()]
    test_files = [diff_files[i] for i in test_idx.tolist()]
